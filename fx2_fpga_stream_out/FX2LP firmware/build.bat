...
REM #
REM # $Archive: /USB/Examples/Fx2lp/slave/build.bat $
REM # $Date: 9/01/03 8:53p $
REM # $Revision: 3 $
REM #
REM #
REM #-----------------------------------------------------------------------------
REM # Copyright 2010, Cypress Semiconductor Corporation
REM #-----------------------------------------------------------------------------
...
c51 slave.c db oe code small moddp2 noiv
...
echo fw.obj, dscr.obj, slave.obj, > tmp.rsp
echo USBJmpTb.obj, >> tmp.rsp
echo EZUSB.lib  >> tmp.rsp
echo TO slave RAMSIZE(256) PL(68) PW(78) CODE(80h) XDATA(1000h)  >> tmp.rsp
bl51 @tmp.rsp

REM ### Generate intel hex image of binary (no debug info) ###
oh51 slave HEXFILE(slave.hex)
